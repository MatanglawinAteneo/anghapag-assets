const scenes = [
    {
      ...
      bgColor: "#0d0101",
      music: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/Kanibalismo.mp3",
      volume: 1.0,
      textPosition: "top",
      textStyle: "",
      textColor: "#e8b89a",
      textOffsetY: "1.35rem",
      uiTopPadding: "3.4rem",
      text: "Tinig: Ihanda ang sarili sa pamamanata. Buong loob na isumpa ang sarili sa kahingian ng Hapag.",
      typing: false,
      showRitual: true,
      typingRitual: true,
      ritualQuestion: "Panata ng Pagkakaisa:\n\"Isusuko ko ang aking sariling daan upang sumunod sa iisang Hapag.\"",
      ritualPlaceholder: "",
      hideButton: true,
      fog: true,
      optionButtons: [
        { text: "\n\"Isusuko ko.\"", nextScene: true },
    ...
    },
    {
      ...
      showRitual: true,
      typingRitual: true,
      ritualQuestion: "Panata ng Pagtalima:\n\"Itutuwid ko ang aking tinig ayon sa itinakda ng Hapag.\"",
      ritualPlaceholder: "",
      hideButton: true,
      fog: true,
      optionButtons: [
        { text: "\n\"Itutuwid ko.\"", nextScene: true },
        { text: "Tumakas", redirect: "https://www.matanglawin-ateneo.com/ang-hapag/paninindigan" }
      ]
    },
    {
      ...
      showRitual: true,
      typingRitual: true,
      ritualQuestion: "Panata ng Paghahatol:\n\"Itataboy ko ang lumilihis sa Hapag, at papanatilihin ang kaayusan nito.\"",
      ritualPlaceholder: "",
      hideButton: true,
      fog: true,
      optionButtons: [
        { text: "\n\"Itataboy ko.\"", nextScene: true },
        { text: "Tumakas", redirect: "https://www.matanglawin-ateneo.com/ang-hapag/paninindigan" }
      ]
    },
    {
      ...
      bg: "",
      bgColor: "#000000",
      music: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/ili-ili[strong].mp3",
      volume: 0.7,
      textPosition: "center",
      textStyle: "break-text",
      textHtml:
        "<span class=\"break-time\">9:00 N.U.</span>" +
        "<span class=\"break-line\">Kinaroroonan: sa Hapag</span>" +
        "<span class=\"break-line\" style=\"max-width:38rem;margin:0.12rem auto 0;display:block;\">Suliranin: Labis na pagkatakam sa mas maraming pagkain</span>" +
      ...
      whiteBtns: true,
      optionButtons: [
        { text: "Magsimula muli", redirect: "https://www.matanglawin-ateneo.com/ang-hapag/introduksyon" },
        { text: "Ipalaganap ang Hapag", openShare: true }
      ],
  ...
  const ritualChoiceButtons = document.getElementById("ritualChoiceButtons");
  ...
  function startHallucinationRedirect() {
    ...
    setTimeout(() => {
      window.location.href = "https://www.matanglawin-ateneo.com/ang-hapag/ikatlong-lunok";
    }, 3200);
  }
  ...
  function setChoiceButtons(scene) {
    // Always clear both containers
    choiceButtons.innerHTML = "";
    choiceButtons.classList.remove("show");
    ritualChoiceButtons.innerHTML = "";
    ritualChoiceButtons.classList.remove("show");

    const options = Array.isArray(scene.optionButtons) ? scene.optionButtons : [];
    if (!options.length) return;

    // Ritual scenes: buttons go inside ritualWrap; break/other scenes: outside
    const target = scene.showRitual ? ritualChoiceButtons : choiceButtons;

    options.forEach((option) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "choice-btn" + (scene.whiteBtns ? " white-btn" : "");
    ...
    if (!scene.typingRitual) {
      target.classList.add("show");
    }
  }

  let ritualTypingInterval = null;

  function setRitualPanel(scene) {
    if (ritualTypingInterval) {
      clearInterval(ritualTypingInterval);
      ritualTypingInterval = null;
    }
    ...
    const hasInput = Boolean(scene.ritualPlaceholder);
    ritualInput.style.display = hasInput ? "" : "none";
    ritualInput.placeholder = scene.ritualPlaceholder || "";
    ritualInput.value = "";
    ritualInput.classList.remove("is-invalid");

    const fullText = scene.ritualQuestion || "";
    const newlineIdx = fullText.indexOf("\n");
    const titleText = newlineIdx !== -1 ? fullText.slice(0, newlineIdx) : fullText;
    const bodyText  = newlineIdx !== -1 ? fullText.slice(newlineIdx + 1) : "";

    function renderRitualHTML(typedTitle, typedBody) {
      ritualQuestion.innerHTML =
        `<span class="ritual-question-title">${typedTitle}</span>` +
        (typedBody ? `<span class="ritual-question-body">${typedBody}</span>` : "");
    }
    ...
    if (scene.typingRitual) {
      ...
      let i = 0;
      ritualTypingInterval = setInterval(() => {
        i += 1;
        const typed = combined.slice(0, i);
        const nl = typed.indexOf("\n");
        const tTitle = nl !== -1 ? typed.slice(0, nl) : typed;
        const tBody  = nl !== -1 ? typed.slice(nl + 1) : "";
        renderRitualHTML(tTitle, tBody);
        if (i >= combined.length) {
          clearInterval(ritualTypingInterval);
          ritualTypingInterval = null;
          ritualChoiceButtons.classList.add("show");
        }
      }, typingSpeedMs);
    } else {
      renderRitualHTML(titleText, bodyText);
    }
  }
  ...
  const whisperSfx = document.getElementById("whisperSfx");
  whisperSfx.volume = 0.50;
  ...
  function setScene(index) {
    ...
    const silhouetteGhost = document.getElementById("silhouetteGhost");
    const candleCanvas = document.getElementById("candleCanvas");
    const isBreak = scene.id === "scene-4-break";
    if (silhouetteGhost) silhouetteGhost.style.display = isBreak ? "none" : "";
    if (candleCanvas) candleCanvas.style.display = isBreak ? "none" : "";
    gameScreen.classList.toggle("break-active", isBreak);
    ...
    if (typeof scene.music === "string" && scene.music.length > 0) {
      const trackChanged = !bgm.src.endsWith(scene.music);
      if (trackChanged) {
        bgm.pause();
        bgm.src = scene.music;
      }
      ...
      if (trackChanged) {
        bgm.volume = scene.volume || bgm.volume;
        bgm.play().catch(() => {});
      } else {
        tryStartMusic();
      }
  ...
  (function () {
    ...
    const ASSETS = [
      ...
      { type: "image", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/dinuguan.png" },
      { type: "image", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/cloche.png" },
      { type: "image", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/silhouettes.PNG" },
      { type: "audio", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/Kanibalismo.mp3" },
      { type: "audio", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/whispering.mp3" },
      { type: "audio", url: "https://raw.githubusercontent.com/MatanglawinAteneo/anghapag-assets/main/CreepyWind.mp3" },
  ...
  (function() {
    ...
    // Multiple candle sources along the bottom, like ritual candles on a table
    const candles = [
      { x: 0.18, y: 0.92, r: 0.38, phase: 0 },
      { x: 0.50, y: 0.95, r: 0.44, phase: 1.8 },
      { x: 0.82, y: 0.92, r: 0.38, phase: 3.5 },
    ];

    function noise(t, seed) {
      return (
        Math.sin(t * 1.7 + seed) * 0.5 +
        Math.sin(t * 3.1 + seed * 1.3) * 0.28 +
        Math.sin(t * 5.4 + seed * 0.7) * 0.14 +
        Math.sin(t * 0.6 + seed * 2.1) * 0.08
      );
    }
    ...
    function draw(now) {
      ...
      candles.forEach((c) => {
        const flicker = 0.55 + 0.45 * (noise(t, c.phase) * 0.5 + 0.5);
        const cx = W * c.x;
        const cy = H * c.y;
        const r  = Math.min(W, H) * c.r * flicker;

        const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
        grad.addColorStop(0,   `rgba(255, 180, 60,  ${0.22 * flicker})`);
        grad.addColorStop(0.2, `rgba(200, 80,  10,  ${0.14 * flicker})`);
        grad.addColorStop(0.5, `rgba(140, 20,  0,   ${0.07 * flicker})`);
  ...
  // ── Share modal ──────────────────────────────────────────────
  const SHARE_URL  = "https://www.matanglawin-ateneo.com/ang-hapag/introduksyon";
  const SHARE_TEXT = "Sumapi na rin ako sa Hapag. Inaanyayahan kitang saluhan kami sa kabusugang magpakailanman" + SHARE_URL;
  ...
  shareFb.href = "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(SHARE_URL) + "&quote=" + encodeURIComponent(SHARE_TEXT);
  shareX.href  = "https://twitter.com/intent/tweet?text=" + encodeURIComponent(SHARE_TEXT);
